...
import fr.smart_builders.port.OwenInboundPort;
...
@OfferedInterfaces (offered = {OwenI.class})
public class 			Owen
extends 				AbstractComponent
{
					...
					throws Exception
	{
		...
		PortI pibp = new OwenInboundPort(inboundport, this);
		pibp.publishPort();
	...
	public boolean switchToEcoMode () {
		this.maximum_consuption = Owen.MAX_ECO_MODE_RUN_CONSUMPTION;
		this.maximum_temp = Owen.ECO_TEMP;
		return this.maximum_consuption == Owen.MAX_ECO_MODE_RUN_CONSUMPTION 
						&& this.maximum_temp == Owen.ECO_TEMP;
	}
	
	public boolean switchToNormalMode () {
		this.maximum_consuption = Owen.MAX_NORMAL_MODE_RUN_CONSUMPTION;
		this.maximum_temp = Owen.MAX_TEMP;
		return this.maximum_consuption == Owen.MAX_NORMAL_MODE_RUN_CONSUMPTION
						&& this.maximum_temp == Owen.MAX_TEMP;
	}
	...
	private void lifeCycle () {
		...
		this.scheduleTask (
				new AbstractComponent.AbstractTask () {
					...
					public void run () {
						try {
							((Owen) this.getTaskOwner()).lifeCycle();
						}catch (Exception e) {
	...
	{
		try {
			PortI [] pf = this.findPortsFromInterface(OwenI.class);
			pf[0].unpublishPort();
